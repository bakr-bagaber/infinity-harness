# Evaluator Rubric — infinity-harness v1.1.0 F6 Review (2026-08-22)
...
Fixed point: `HEAD` (feature/infinity-harness-f7-ops) vs `b37b557` (main v1.1.0). F7 delta is ops activation, not scope creep.
...
- **F1-F5** — already shipped and validated on main (see `DECISIONS.md` F2-F5, `CHANGELOG.md` 0.2.0..1.0.0, `npx tsc` clean, `the infinity_validate tool SHIP 9/9`). No regression in F7 (1324->1359 lines enforcer, tsc 0, remote still read-only, worker isolation intact).
...
- **Large Class/Method** — `extensions/infinity-harness/index.ts` 1359 lines, but split by Pi event (session_start/context/turn_end/tool_call/agent_start) and hidden tools (infinity_plan, infinity_spawn_worker, infinity_goal, infinity_dashboard); F7 added 35 lines turn_end auto-wire only, no growth beyond single responsibility per handler.
...
- Wired `extensions/infinity-harness/index.ts:turn_end` auto-bounce (review) + unstuck suggest (budgets/hysteresis/fileDelta), notify+appendEntry only.
- Synced `harness/config.json:git.branch` to current branch `feature/infinity-harness-f7-ops` (was stale f6-resilient); .gitignore keeps it runtime, but local file matches git for hygiene.
